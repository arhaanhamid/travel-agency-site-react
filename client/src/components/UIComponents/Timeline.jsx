...
export const Timeline = ({ data }) => {
  ...
  return (
    <div className="w-full md:px-10" ref={containerRef}>
      <div className="max-w-7xl mx-auto py-20 px-4 md:px-8 lg:px-10 lazyLoadDown">
        <h2 className="text-lg md:text-4xl mb-4 text-indigo-600 max-w-4xl">
          Our future awaits us!
        </h2>
        <p className="text-indigo-400 text-sm md:text-base max-w-sm">
        ...
        {data.map((item, index) => (
          ...
          >
            <div className="sticky flex flex-col md:flex-row z-40 items-center top-40 self-start max-w-xs lg:max-w-sm md:w-full">
              ...
              <h3 className="hidden md:block text-xl md:pl-20 md:text-5xl font-bold text-indigo-400 lazyLoadLeft">
                {item.title}
              </h3>
            </div>

            <div className="relative pl-20 pr-4 md:pl-4 w-full">
              <h3 className="md:hidden block text-2xl mb-4 text-left font-bold text-indigo-400 lazyLoadLeft">
                {item.title}
              </h3>
              <p className="text-indigo-600 lazyLoadRight">{item.desc}</p>
            </div>
          </div>
        ))}
        <div
